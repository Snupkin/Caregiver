...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    for (i = 0; i < 6; i++) {
        if (indexPath.row == i && i < 3) {
            cell.backgroundColor = [UIColor colorWithRed:5.0/255.0f green:255.0/255.0f blue:5.0/255.0f alpha:0.3f]; // Some green color
        }
        else if (indexPath.row == i && i >= 3 && i < 6){
            cell.backgroundColor = [UIColor colorWithRed:45.0/255.0f green:100.0/255.0f blue:245.0/255.0f alpha:0.3f];
        }
    }
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    ...
    if ([sender isKindOfClass:[UITableViewCell class]]) {
        if ([segue.destinationViewController isKindOfClass:[ResearchViewController class]]) {
            ...
            researchVC.labArray = self.labArray;
        }
    }
}
